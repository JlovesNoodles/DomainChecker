...
while true; do
    ...
    function domainchecker() {
        ...
        if [[ $user_choice == "G" || $user_choice == "g" ]]; then
            echo "Checking if the Domain is using Google" | lolcat
            ...
            cat results/googleresults/googleusingdomain.txt  | lolcat
            echo " "
            echo " "
            echo "printing domain not using google"
            cat results/googleresults/notgoogleusingdomain.txt | lolcat
            ...
            while read -r domain; do
                ...
                if [[ -n "$result" ]]; then 
                    echo "$domain is associated with Microsoft." >> results/microsoftresults/microsoftusingdomain.txt | lolcat
                else
                    echo "$domain is not associated with Microsoft." >> results/microsoftresults/notmicrosoftusingdomain.txt | lolcat
                fi
            done < domain_list.txt
...
domainchecker
    read -p "Do you want to check another set of domains? (Y/N): " continue_checking | lolcat

    if [[ $continue_checking != "Y" || $continue_checking != "y" ]]; then
        echo "Adios Amigo!" | lolcat
        break
    fi
done
